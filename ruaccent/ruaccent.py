...
class RUAccent:
    ...
    def load(
        ...
        self.omograph_model.load(join_path(self.workdir, self.omograph_models_paths[omograph_model_size][1:]), providers=providers)

        self.yo_words = json.load(
            gzip.open(join_path(self.workdir, "dictionary","yo_words.json.gz"))
        ) 
        self.accent_model.load(join_path(self.workdir, "nn","nn_accent/"), providers=providers)
        self.yo_homographs = json.load(
                gzip.open(join_path(self.workdir, "dictionary","yo_homographs.json.gz"))
            ) 
        self.yo_homograph_model.load(join_path(self.workdir, "nn","nn_yo_homograph_resolver"), providers=providers)
        ...
        if not self.tiny_mode:
            ...
            self.stress_usage_predictor.load(join_path(self.workdir, "nn","nn_stress_usage_predictor/"), providers=providers)
